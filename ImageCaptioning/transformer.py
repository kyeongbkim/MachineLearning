...
class Transformer(tf.keras.Model):
  ...
  def call(self, inp, target, training, look_ahead_mask=None, dec_padding_mask=None,enc_padding_mask=None):
...
class ImageCaptioningWithTransformer(ImageCaptioning):
    def __init__(self, dataset_name=None):
        ...
        self.set_model_name_prefix('{}-transformer'.format(dataset_name))
    ...
    def build_model(self):
        ...
        model = Transformer(self.num_layer, self.d_model, self.num_heads, self.dff,
    ...
    def train_step(self, img_tensor, target):
        ...
        with tf.GradientTape() as tape:
            predictions, _ = self.model(img_tensor, tar_inp, True,  dec_mask)
    ...
    def train(self, total_epochs, batch_size=64, save_model=False):
        ...
        for epoch in range(completed_epoch, total_epochs):
            ...
            for batch, total_num_batches, img_tensor, target in data_generator:
                self.train_step(img_tensor, target)
                num_steps += 1

                is_last = (batch + 1) == total_num_batches
                if num_steps % 50 == 0 or is_last:
                    print ('\r    Batch {}/{} Loss {:.4f} Accuracy {:.4f}'.format(
                               batch+1, total_num_batches,
                               self.train_loss.result(), self.train_accuracy.result()), end='')

            self.callback.on_epoch_end(epoch=epoch, logs={'loss': self.train_loss.result()} )
            print ('\r\n    Epoch {} Loss {:.4f} Accuracy {:.4f}, Time {:.1f} secs'.format(
    ...
    def predict(self, image_id):
        ...
        for i in range(max_length):
            ...
            predictions, attention_weights = self.model(img_tensor, output, False, dec_mask)
